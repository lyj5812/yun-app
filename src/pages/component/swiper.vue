<template>
  <view>
    <cu-custom bg-color="bg-gradual-pink" :is-back="true"><block slot="backText">返回</block>
    ...
    <view class="cu-bar bg-white">
      ...
      <view class="action">
        <switch :class="dotStyle?'checked':''" :checked="dotStyle?true:false" @change="DotStyle" />
      </view>
    </view>
    <swiper
      class="screen-swiper"
    ...
    >
      <swiper-item v-for="(item,index) in swiperList" :key="index">
        ...
        <video v-if="item.type=='video'" :src="item.url" autoplay loop muted :show-play-btn="false" :controls="false" objectFit="cover" />
      </swiper-item>
    </swiper>
    <!-- #ifndef MP-ALIPAY -->
    ...
    <view class="tower-swiper" @touchmove="TowerMove" @touchstart="TowerStart" @touchend="TowerEnd">
      <view v-for="(item,index) in swiperList" :key="index" class="tower-item" :class="item.zIndex==1?'none':''" :style="[{'--index': item.zIndex,'--left':item.mLeft}]" :data-direction="direction">
        <view class="swiper-item">
          <image v-if="item.type=='image'" :src="item.url" mode="aspectFill" />
          <video v-if="item.type=='video'" :src="item.url" autoplay loop muted :show-play-btn="false" :controls="false" objectFit="cover" />
        </view>
