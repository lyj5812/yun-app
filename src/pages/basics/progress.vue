<template>
  <view>
    <cu-custom bg-color="bg-gradual-blue" :is-back="true"><block slot="backText">返回</block><block slot="content">进度条</block></cu-custom>
    ...
    <view class="cu-bar bg-white solid-bottom margin-top" data-target="ColorModal" @tap="showModal">
      <view class="action">
    ...
    <view class="cu-bar bg-white solid-bottom margin-top">
      ...
      <switch class="margin-right-sm" :class="active?'checked':''" @change="SetActive" />
    </view>
    <view class="padding bg-white">
    ...
    <view class="cu-modal" :class="modalName=='ColorModal'?'show':''">
      <view class="cu-dialog">
        ...
        <view class="grid col-5 padding">
          <view v-for="(item,index) in ColorList" v-if="item.name!='gray' && item.name!='white'" :key="index" class="padding-xs" :data-color="item.name" @tap="SetColor">
