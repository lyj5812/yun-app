<template>
  <view>
    <cu-custom bg-color="bg-gradual-blue" :is-back="true"><block slot="backText">返回</block><block slot="content">按钮</block></cu-custom>
    <view class="cu-bar bg-white solid-bottom">
      <view class="action">
        <text class="cuIcon-title text-blue" />按钮形状
      </view>
      <view class="action">
        <navigator class="action" url="design" hover-class="none">
          <text class="cuIcon-skinfill" />
    ...
    <view class="padding flex flex-wrap justify-between align-center bg-white">
      ...
      <button class="cu-btn cuIcon">
        <text class="cuIcon-emojifill" />
      </button>
    </view>
    <view class="cu-bar margin-top bg-white solid-bottom">
      <view class="action">
        <text class="cuIcon-title text-blue" />按钮尺寸
      </view>
    </view>
    <view class="padding flex flex-wrap justify-between align-center bg-white">
    ...
    <view class="cu-bar margin-top bg-white">
      <view class="action">
        <text class="cuIcon-title text-blue" />按钮颜色
      </view>
      <view class="action">
        <text class="text-df margin-right-sm">阴影</text>
        <switch :class="shadow?'checked':''" color="#39B54A" @change="SetShadow" />
      </view>
    </view>
    <view class="grid col-5 padding-sm">
      <view v-for="(item,index) in ColorList" :key="index" class="margin-tb-sm text-center">
        <button class="cu-btn round" :class="['bg-' + item.name , shadow?'shadow':'']">{{ item.title }}</button>
      </view>
    </view>
    <view class="cu-bar margin-top bg-white">
      <view class="action">
        <text class="cuIcon-title text-blue" />镂空按钮
      </view>
      <view class="action">
    ...
    <view class="grid col-5 padding-sm">
      <view v-for="(item,index) in ColorList" v-if="item.name!='white'" :key="index" class="margin-tb-sm text-center">
        <button class="cu-btn round" :class="[bordersize?'lines-' + item.name:'line-' + item.name, shadow?'shadow':'']">{{ item.title }}</button>
      </view>
    </view>
    <view class="cu-bar margin-top bg-white">
      <view class="action">
        <text class="cuIcon-title text-blue" />块状按钮
      </view>
    </view>
    ...
    <view class="cu-bar margin-top bg-white">
      <view class="action">
        <text class="cuIcon-title text-blue" />无效状态
      </view>
    </view>
    <view class="padding">
    ...
    <view class="cu-bar margin-top bg-white">
      <view class="action">
        <text class="cuIcon-title text-blue" />按钮加图标
      </view>
    </view>
    <view class="padding-xl">
      <button class="cu-btn block line-orange lg">
        <text class="cuIcon-upload" /> 图标</button>
      <button class="cu-btn block bg-blue margin-tb-sm lg">
        <text class="cuIcon-loading2 cuIconfont-spin" /> 加载</button>
